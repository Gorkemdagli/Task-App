 ...
 */
export function getDisplayStatus(task: Task, currentUserId: string | undefined): TaskStatus {
  if (task.pendingStatus && currentUserId && task.pendingProposer?.id === currentUserId) {
 ...
 */
export function isPendingForViewer(task: Task, currentUserId: string | undefined): boolean {
